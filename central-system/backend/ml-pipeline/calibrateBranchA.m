...
% ── The same constants, as JSON ────────────────────────────────────────────
% Branch A inference runs in Python (the MATLAB PyTorch import produces wrong
% numbers — see testImportedNetwork.m), so the serving side cannot read a .mat.
% Writing both formats from ONE fit is what keeps them from drifting: a second
...
calJson = struct( ...
    ...
    'referableFrom',  2, ...
    'fittedOn',       sprintf('val split, n=%d', numel(valLabels)), ...
    'fittedAt',       char(datetime('now','TimeZone','UTC','Format','yyyy-MM-dd''T''HH:mm:ss''Z''')), ...
    'note',           ['fitted by calibrateBranchA.m. Do not hand-edit: ' ...
                       'temperature and qhat come from one fit on one fold ' ...
                       'and are only jointly meaningful.']);
jsonPath = fullfile(modelsDir, 'calibration_v1.json');
