// Adapted anatomy for the central-system login — compressed scroll stages.
// Fewer chapters, faster transitions, stiffer springs.
import * as THREE from 'three';
import { RoomEnvironment } from 'three/addons/environments/RoomEnvironment.js';
import { buildEye } from './model.js';
import { RETINA_R, DISC } from './textures.js';
import { clamp, lerp, ease, smoothstep, smoothDamp } from './math.js';
import { FRONT_FOV, GLOBE_MM, frontMetrics } from './frame.js';

// Extended stages — 7 screens for a more deliberate, premium journey
export const STAGES = {
  orbit: [0.8, 2.0],
  explode: [1.6, 3.8],
  labels: [3.5, 5.0],
...
export async function createAnatomy({ host, labelsEl, leadersEl, quality, fundus, reduced = false }) {
  ...
  const labels = LABELS.map((l) => {
    ...
    el.addEventListener('pointerenter', () => { labelHover = l.part; });
    el.addEventListener('pointerleave', () => { if (labelHover === l.part) labelHover = null; });
  ...
  function smoothPose(p, dt) {
    ...
    if (!cam) cam = want;
    // Softer spring for cinematic camera movement
    else for (const k of KEYS) cam[k] = smoothDamp(cam[k], want[k], vel[k], 0.35, dt);
    return cam;
  }
  ...
  function update(s, time, { dt = 1 / 60, pointer = null, hover = null, adapt: adaptive = false } = {}) {
    ...
    eye.pivot.rotation.y = 0.05 * Math.sin(time * 0.3) * smoothstep(0.6, 1, s) * (1 - r);
    ...
    if (pointer) {
      const k = smoothstep(0.7, 1.0, s) * (1 - 0.8 * f);
  ...
  function placeLabels(s) {
    ...
    labels.forEach((l, i) => {
      const lineIn = smoothstep(a0 + i * 0.04, a0 + 0.18 + i * 0.04, s);
      const textIn = smoothstep(a0 + 0.08 + i * 0.04, a0 + 0.28 + i * 0.04, s);
      const fadeOut = 1 - smoothstep(l.accent ? 3.6 : a1 - 0.15, l.accent ? 3.8 : a1, s);
