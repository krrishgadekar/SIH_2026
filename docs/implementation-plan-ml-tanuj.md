...
- **ONNX export + MATLAB import already done and parity-verified for all 5 models, not just Branch A.** `branchA_v1`, `vessel_unet_v1`, `localization_v1`, `bright_lesion_unet_v1`, and `red_lesion_unet_v1` were all exported via `torch.onnx.export`, imported into MATLAB via `importNetworkFromONNX`, and checked against 10 real IDRiD images — every one came back with max-difference parity between 2e-6 and 4e-5 against the original PyTorch output. The `+<modelname>/` custom-layer package folders each `.mat` depends on at load time (needed for things like EfficientNet's squeeze-excite blocks) were already identified as a real deployment gotcha and already copied into `models/` alongside their `.mat` files, confirmed working from a clean session. **You do not need to redo this conversion for M2, M3, or M4** — it's done and verified. Only Branch A is currently wired into the live grading path (`INFERENCE_BACKEND=matlab`); M2/M3/M4's already-converted `.mat` files aren't called from anywhere yet — that's an orchestration/wiring task, and it's Saad's, not yours (see his plan). **M5's conversion is the one exception that will need redoing**: it was done against the *current* 2-class merged model, which the retrain in §4 replaces with a new 3-class architecture — once that retrain lands, repeat the same export→import→parity-check process against the new checkpoint, not the old one.
